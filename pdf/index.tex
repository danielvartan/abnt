...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, frame hidden, boxrule=0pt, breakable, sharp corners, enhanced, colback={codebgcolor}]}{\end{tcolorbox}}\fi
\makeatother
...
\subcaption{\label{fig-gini-index}}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-pdf/fig-gini-index-1.png}}
...
\caption{\label{fig-mpg}Scatter plot with marginal distributions of
weight (\emph{1000 lbs}) and fuel efficiency (\emph{miles per gallon})
for combustion engine vehicles}

\centering{

\subcaption{\label{fig-mpg}}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-pdf/fig-mpg-1.png}}
...
\subcaption{\label{fig-lorenz-system}}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{qmd/development_files/figure-pdf/fig-lorenz-system-1.png}}
...
\subcaption{\label{fig-sao-paulo-tracts}}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{qmd/development_files/figure-pdf/fig-sao-paulo-tracts-1.png}}

}

\legendleft{Tract data from the
...
\pandocbounded{\includegraphics[keepaspectratio]{qmd/development_files/figure-pdf/fig-eruption-1.png}}
...
Sit arcu egestas cum facilisi pharetra sapien litora egestas. Faucibus
libero ullamcorper nisi ac pellentesque rutrum sed rutrum at elementum
mi. Phasellus sapien ornare vulputate molestie lacinia sodales mollis
tristique placerat lectus iaculis molestie eu porta suscipit phasellus
phasellus eu nibh fames mus molestie eros cum suspendisse cum faucibus
nunc ultrices facilisi id a porttitor aliquet leo -- vivamus
pellentesque enim interdum, diam ornare placerat porta -- malesuada,
quisque fermentum arcu eros dictum: mattis dui fringilla, ante hendrerit
...
pellentesque mattis tempor magna pulvinar per turpis velit
...
\begin{tcolorbox}[enhanced jigsaw, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, colframe=quarto-callout-note-color-frame, colbacktitle=quarto-callout-note-color!10!white, opacityback=0, opacitybacktitle=0.6, bottomtitle=1mm, colback=white, titlerule=0mm, bottomrule=.15mm, rightrule=.15mm, toptitle=1mm, coltitle=black, leftrule=.75mm, breakable, left=2mm, toprule=.15mm, arc=.35mm]
...
This format uses \href{https://www.luatex.org/}{\texttt{lualatex}}. I
recommend always using this engine for better font handling and feature
support.
...
\href{https://quarto.org/docs/reference/formats/pdf.html}{PDF options}
like \texttt{mainfont}, \texttt{monofont}, and \texttt{sansfont}.
...
\subsection{Font Size and Line-Height}\label{font-size-and-line-height}

Use \texttt{fontsize} and \texttt{linestretch} in \texttt{\_quarto.yml}
...
Note that Section 5.1 of the ABNT NBR 14724:2011 norm specifies that the
font size should be \textbf{12pt} for the entire document, including the
cover, except for quotations longer than three lines, footnotes,
pagination, cataloging data, captions, and sources of illustrations and
tables, which should be in a smaller and uniform size.

\section{Language and Hyphenation}\label{language-and-hyphenation}

\index{language}

This format includes built-in support for multiple languages and
hyphenation patterns, which are automatically loaded based on the
document's language setting.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{format}\KeywordTok{:}
\AttributeTok{  }\FunctionTok{abnt{-}pdf}\KeywordTok{:}
\AttributeTok{    }\FunctionTok{lang}\KeywordTok{:}\AttributeTok{ en}
\AttributeTok{    }\FunctionTok{babel{-}lang}\KeywordTok{:}\AttributeTok{ english}
\AttributeTok{    }\FunctionTok{other{-}babel{-}lang}\KeywordTok{:}\CommentTok{ \# Do not repeat \textquotesingle{}babel{-}lang\textquotesingle{} here.}
\AttributeTok{      }\KeywordTok{{-}}\AttributeTok{ french}
\AttributeTok{      }\KeywordTok{{-}}\AttributeTok{ spanish}
\AttributeTok{      }\KeywordTok{{-}}\AttributeTok{ brazil}
\end{Highlighting}
\end{Shaded}
...
Exercise caution when removing post-textual sections (e.g., Glossary,
Appendices, Annexes). Each section begins and ends with specific LaTeX
commands (e.g., \texttt{\textbackslash{}begin\{anexosenv\}}).
...
\tightlist
\item
  Delete it from \texttt{\_quarto.yml}
\item
  Remove the \texttt{\textbackslash{}begin\{glossario\}} LaTeX code at
  the end of the \emph{Conclusion} or last chapter
\item
  Copy the LaTeX code following
  \texttt{\textbackslash{}begin\{apendicesenv\}} in
  \texttt{glossary.qmd} to the bottom of the \emph{Conclusion} or last
  chapter
\end{enumerate}

The same process applies when removing Appendices and Annexes. The last
appendix must include LaTeX code to close the section and initialize the
Annexes section.

The Index section is an exception. To remove it, add the following to
your \texttt{\_quarto.yml} file:
...
\AttributeTok{      }\KeywordTok{{-}}\AttributeTok{ isbn=true}
...
\section{LaTeX Commands in Quarto
Files}\label{latex-commands-in-quarto-files}

When adding LaTeX commands in Quarto always use a code block with
\texttt{\{=latex\}}:
...
\texttt{div} enclosures. For example:

See Theorem~\ref{thm-line}.
...
Each type of \texttt{div} must use pre-defined prefixes. If you don't
follow these rules, your document may behave unexpectedly.
...
execution options. If you're having issues updating your document, check
whether these options are enabled in your \texttt{\_quarto.yml} file.
...
\AttributeTok{  }\FunctionTok{cache}\KeywordTok{:}\AttributeTok{ }\CharTok{false}\CommentTok{ \# "Danger, Will Robinson!"}
\AttributeTok{  }\FunctionTok{freeze}\KeywordTok{:}\AttributeTok{ }\CharTok{false}\CommentTok{ \# If you plan to use it, remember to disable it afterward!}
\end{Highlighting}
\end{Shaded}
