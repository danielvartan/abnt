...
with_seed(
  ...
  code = {
    ipsum(1, avg_words_per_sentence = 22) |>
      as.character() |>
      str_remove(".$") |>
      paste0(" [@krakauer2019; @mitchell2009a].") |>
      cat(sep = "\n\n")
  }
)
```
...
brazil_shape <-
  here("data", "geobr-read_country-2010.rds") |>
  read_rds()
```
...
municipalities_shape <-
  here("data", "geobr-read_municipality-2010.rds") |>
  read_rds()
```
...
Scatter plot with marginal distributions of weight (*1000 lbs*) and fuel efficiency (*miles per gallon*) for combustion engine vehicles
:::
...
tibble(
  ...
  cols_width(
    decision ~ pct((1 / 3) * 100),
    h0_true ~ pct((1 / 3) * 100),
    h0_false ~ pct((1 / 3) * 100)
  ) |>
  fmt_markdown() |>
  tab_style(
    style = cell_text(weight = "bold"),
