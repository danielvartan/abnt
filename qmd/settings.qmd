...
If you won't use a section of this document, you must remove it from `_quarto.yml`. Empty sections will produce an error.

At the moment, you also need to remove them in `tex/include-before-body.tex` and `R/.pre-render.R`.
...
To add LaTeX commands in your writing use a `{=latex}` chunk.
...
This Quarto format was specifically designed to be compatible with  [BibLaTeX](https://www.ctan.org/pkg/biblatex), which is a comprehensive re-implementation of [BiBTeX](https://www.bibtex.org/). At first glance, these two systems may appear very similar.
...
Visit the showcase chapter "Introduction" (`qmd/introduction.qmd`) of this Quarto format to see this feature in action. For more detailed information, please refer to Quarto's [Cross-referenceable elements](https://quarto.org/docs/prerelease/1.4/crossref.html) article.
...
See [Template partials](https://quarto.org/docs/journals/templates.html#template-partials).
...
Use `\the\baselineskip` and `\getvalue{}` to figure out the exact value. Note that `\getvalue{}` will return the value in `pt`.
